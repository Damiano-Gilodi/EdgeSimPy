...
class Application(ComponentManager, Agent):
    ...
    def __init__(self, obj_id: int | None = None, label: str = ""):
        ...
        self.user_data_packets: dict[str, list["DataPacket"]] = {}
    ...
    def register_data_packet(self, user: "User", size: int = 1) -> "DataPacket":
        ...
        Args:
            ...
            size (int, optional): Size of the data packet in bytes. Defaults to 1.

        Returns:
            object: Created DataPacket object.
        """
        dp = DataPacket(user=user, application=self, size=size)
        if str(user.id) not in self.user_data_packets:
            self.user_data_packets[str(user.id)] = []
        self.user_data_packets[str(user.id)].append(dp)
        return dp
